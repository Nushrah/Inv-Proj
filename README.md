...
## Optional: IDE-only development (not the submission path)

If you want hot reload while coding, you can run Postgres via Compose and start `npm run dev` in `backend/` and `frontend/` with env vars from `backend/.env.example`. **For submission and reviewers, use `docker compose up --build` as above.**

## Git

Keep a clear commit history. Do not commit secrets; use local `.env` only when developing outside Compose.
